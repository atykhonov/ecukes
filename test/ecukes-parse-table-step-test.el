...
(ert-deftest parse-table-step-single-row ()
  ...
  (with-parse-step
   "table-single-row"
   (lambda (name head body type args)
     (should (eq type 'table))
     (should (equal (car args) '(("meal" "price") ("Hamburger" "$4.50")))))))
...
(ert-deftest parse-table-step-multiple-rows ()
  ...
  (with-parse-step
   ...
   (lambda (name head body type args)
     (should (eq type 'table))
     (should (equal (car args) '(("meal" "price") ("Hamburger" "$4.50") ("Pizza" "$5.30")))))))
...
(ert-deftest parse-table-step-same-row ()
  ...
  (with-parse-step
   ...
   (lambda (name head body type arg)
     (should (eq type 'table))
     (should (equal (car args) '(("meal" "price") ("Hamburger" "$4.50") ("Hamburger" "$4.50")))))))
...
(ert-deftest parse-table-step-empty-columns ()
  ...
  (with-parse-step
   ...
   (lambda (name head body type arg)
     (should (eq type 'table))
     (should (equal (car args) '(("meal" "price") ("Hamburger" "") ("Pizza" "$5.30")))))))
