...
(ert-deftest parse-table-step-single-row ()
  ...
  (with-parse-step
   ...
   (lambda (name head body type args pystring table)
     ...
     (should (equal table '(("meal" "price") ("Hamburger" "$4.50")))))))
...
(ert-deftest parse-table-step-multiple-rows ()
  ...
  (with-parse-step
   ...
   (lambda (name head body type args pystring table)
     (should (eq type 'table))
     (should (equal table '(("meal" "price") ("Hamburger" "$4.50") ("Pizza" "$5.30")))))))
...
(ert-deftest parse-table-step-same-row ()
  ...
  (with-parse-step
   "table-same-row"
   (lambda (name head body type arg pystring table)
     (should (eq type 'table))
     (should (equal table '(("meal" "price") ("Hamburger" "$4.50") ("Hamburger" "$4.50")))))))
...
(ert-deftest parse-table-step-empty-columns ()
  ...
  (with-parse-step
   ...
   (lambda (name head body type arg pystring table)
     (should (eq type 'table))
     (should (equal table '(("meal" "price") ("Hamburger" "") ("Pizza" "$5.30")))))))
